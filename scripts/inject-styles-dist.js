...
function injectStylesheet(htmlPath, cssPath) {
  ...
  if (html.includes('archive-styles.css')) {
    console.log(`✓ Already styled: ${path.relative(process.cwd(), htmlPath)}`);
    return false;
  }
  ...
  console.log(`✓ Styled: ${path.relative(process.cwd(), htmlPath)}`);
  return true;
}

// Main execution
const distArchiveDir = path.join(__dirname, '..', 'ipip-site', 'dist', 'archive');
const cssFile = path.join(distArchiveDir, 'archive-styles.css');

console.log('🎨 Injecting unified styles into dist archive HTML files...\n');
...
console.log(`\n✨ Done! Modified ${modifiedCount} of ${htmlFiles.length} HTML files in dist.`);
